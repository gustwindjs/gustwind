import { setupTwind, sheets } from "../deps.ts";
import type {
  Components,
  DataContext,
  Layout,
...
const stylesheet = sheets.virtualSheet();
...
async function renderPage({
  ...
  try {
    ...
    const styleTag = projectMeta.features?.extractCSS
      ...
      : sheets.getStyleTag(stylesheet);

    let css = "";
    if (projectMeta.features?.extractCSS) {
      css = sheets.getStyleTagProperties(stylesheet).textContent;
    }
